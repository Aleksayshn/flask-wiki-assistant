...
### Flask starts but remote lookup fails

Check:

- `REMOTE_EXECUTION_ENABLED=true`
- `EC2_HOST` is correct
- `EC2_USERNAME` is correct
- `EC2_KEY_PATH` points to the correct SSH private key
- `EC2_WIKI_SCRIPT_PATH` exists on EC2

### Remote error: `ModuleNotFoundError: No module named 'wikipedia'`

This usually means the remote script is being run with the wrong Python
interpreter or the package is not installed inside the EC2 virtual
environment.

Verify on EC2:

```bash
/home/ubuntu/wiki-service/venv/bin/python -m pip show wikipedia
```
...
- `MYSQL_CACHE_ENABLED=true`
- VM 2 is reachable from VM 1
- MySQL container is running
- port `7888` is open
- database credentials in `.env` are correct

### App still works when MySQL is down

This is expected behaviour. The application logs a warning and falls back to
remote lookup instead of stopping completely.

### SSH works manually but Flask still fails

Test the EC2 command manually from VM 1:
...
If this works, compare the result with the values in `.env`.

## Notes For the Lecturer / Demo

During the demo, the following points can be shown clearly:

- the web interface runs on VM 1 with Flask
- the search request first checks MySQL on VM 2
- the first request for a term triggers a remote lookup on EC2
- the second request for the same term is served from the cache
- the Flask console logs show cache hit, cache miss, remote search start, remote search completion, and cache save status
- if MySQL is unavailable, the web app still runs and can continue using remote lookup

Suggested demo sequence:

1. Start Flask and show the startup logs.
2. Search for a term that is not already cached.
3. Explain that Flask connects to EC2 over SSH.
4. Show the returned result in the browser.
5. Search for the same term again.
6. Show that the second response comes from the cache.

## Submission Notes

This project was written with a modular structure to make the architecture
easy to explain and extend:

- `app/main.py` controls the Flask routes and workflow
- `app/remote_wiki.py` handles SSH execution and output parsing
- `app/db.py` handles MySQL caching
- `scripts/wiki.py` performs the Wikipedia lookup on EC2

The code is intentionally clear and commented so it is suitable for academic
review, marking, and live demonstration.
